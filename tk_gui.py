...
class GUI(Tk):
    ...
    def __init__(self, master=None):
        ...
        self.title('Taiko Master v0.3.4')
        ...
        # self._canvas = {
        #     'taiko_img': Canvas(master, height=321, width=479)
        #     }
        self._labels = {
            'drummer_name': Label(master, text='Drummer\'s name:'),
            'song_id': Label(master, text='Song ID:'),
            'taiko_img': Label(master, image=ImageTk.PhotoImage(Image.open('taiko.jpg')))
        }
    ...
    def __create_image(self):
        #self._labels['taiko_img'].place(x=0, y=0)
        self._labels['taiko_img'].pack()
    # def __create_canvas(self):
    #     im=Image.open("taiko.jpg")
    #     img_file=ImageTk.PhotoImage(im)
    #     can_img1=self._canvas['taiko_img']
    #     image=can_img1.create_image(0,0,anchor='nw', image=img_file)
    #     can_img1.pack(side='top')
...
if __name__ == "__main__":
    ...
    imLabel=Label(window, image=img).place(x=700,y=0)
    window.mainloop()
